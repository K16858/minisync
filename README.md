...
トークンを明示的に指定する場合：
```bash
msync push myfile.txt --host 192.168.1.100 --port 61001 --token a59f2921c9b101499d001ac005dad7bc
```

上書き確認をスキップする場合：
```bash
msync pull myfile.txt --yes
```

## 認証の仕組み

minisyncは簡易的なトークンベース認証を使用します。

### トークンの生成と保存

`msync init`を実行すると：
1. 32文字のランダムなトークンが自動生成される
2. `.msync/config.json`に保存される
3. 同じspaceを持つ全てのマシンが同じトークンを共有する

### 認証フロー

1. クライアントがサーバーに接続
2. クライアントが自分のspace IDを送信（HELLO）
3. サーバーがspace IDで該当spaceを特定
4. クライアントがトークンを送信（TOKEN）
5. サーバーが該当spaceの`.msync/config.json`からトークンを読み込んで照合
6. トークンが一致すれば認証成功、不一致ならエラー
...
同じspaceを複数マシンで使用する場合、`.msync/config.json`を安全にコピーしてください。
